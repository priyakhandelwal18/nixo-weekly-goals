...
import { Milestone, Goal, TeamMember } from '@/types';
...
interface MilestonesListProps {
  ...
  onToggleAchieved: (milestoneId: string, achieved: boolean) => void;
...
export function MilestonesList({
  ...
  const getLinkedGoals = (milestone: Milestone): Goal[] => {
    return goals.filter((g) => g.milestoneId === milestone.id);
  };
    ...
    return (
      <div className="flex-1 flex items-center justify-center text-gray-500">
        <div className="text-center">
          ...
          <p className="text-sm mt-1">Add a milestone to track bigger goals with deadlines</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="relative">
          ...
          {milestones.map((milestone) => {
            const linkedGoals = getLinkedGoals(milestone);
            const isExpanded = expandedId === milestone.id;
            const overdue = isOverdue(milestone.deadline) && !milestone.achieved;
              ...
              <div key={milestone.id} className="relative pl-12 pb-8">
                ...
                <div
                  className={`bg-white border rounded-lg p-4 cursor-pointer transition-shadow hover:shadow-md ${
                    milestone.achieved ? 'border-green-200' : overdue ? 'border-red-200' : 'border-gray-200'
                  }`}
                  ...
                  <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        <h3
                          className={`font-medium text-lg ${
                            milestone.achieved ? 'text-gray-500 line-through' : 'text-gray-900'
                          }`}
                        >
                          {milestone.title}
                        </h3>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          onToggleAchieved(milestone.id, !milestone.achieved);
                        }}
                      ...
                      >
                        {milestone.achieved ? 'Mark Not Achieved' : 'Mark Achieved'}
                      </button>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
